...
# ===================== DENEYLER =====================================
# v10 Optuna'sinin buldugu LGB parametreleri
BEST_LGB = {
    "objective": "binary", "verbose": -1, "bagging_freq": 1,
    "learning_rate":     0.02019740641455602,
    "num_leaves":        28,
...
BASE = dict(BEST_LGB)   # v10-tuned parametreler
def lb_score(yy,pp): return 0.4*roc_auc_score(yy,pp) - log_loss(yy,pp)

def run_cv(Xd, Xtd=None, pseudo=False, auc_stop=False, seed=42):
    """5-fold LGB; pseudo=True ise her foldda test'in emin tahminleri egitime katilir."""
    oof=np.zeros(len(Xd))
    met = ["auc","binary_logloss"] if auc_stop else ["binary_logloss","auc"]
    p=dict(BASE, metric=met, seed=seed)
    skf=StratifiedKFold(5,shuffle=True,random_state=seed)
    for tr,va in skf.split(Xd,y):
        dtr=lgb.Dataset(Xd.iloc[tr],y.iloc[tr])
        m=lgb.train(p,dtr,num_boost_round=6000,
            valid_sets=[lgb.Dataset(Xd.iloc[va],y.iloc[va])],
            callbacks=[lgb.early_stopping(200,first_metric_only=True),lgb.log_evaluation(0)])
        if pseudo:
            pt=m.predict(Xtd)
            sel=(pt<0.02)|(pt>0.90)
            Xp=pd.concat([Xd.iloc[tr],Xtd[sel]],ignore_index=True)
            yp=pd.concat([y.iloc[tr],pd.Series((pt[sel]>0.5).astype(int))],ignore_index=True)
            wp=np.r_[np.ones(len(tr)),np.full(sel.sum(),0.6)]
            m=lgb.train(p,lgb.Dataset(Xp,yp,weight=wp),num_boost_round=6000,
                valid_sets=[lgb.Dataset(Xd.iloc[va],y.iloc[va])],
                callbacks=[lgb.early_stopping(200,first_metric_only=True),lgb.log_evaluation(0)])
...
print("\n"+"="*70)
print("E1) URETECI TERSINE MUHENDISLIK — egitim yok, saf istatistik")
print("="*70)
r = train["bal_recent_over_old"]
print(f"\nbal_recent_over_old -> stres orani (ince aralik, taban {y.mean():.3f}):")
bins=[0,0.4,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.0,1.05,1.1,1.2,10]
tab=y.groupby(pd.cut(r,bins)).agg(["mean","count"])
print(tab.round(3).to_string())
print(f"\nbal_slope6_norm -> stres orani (desiller):")
print(y.groupby(pd.qcut(train["bal_slope6_norm"],10,duplicates="drop")).agg(["mean","count"]).round(3).to_string())
print(f"\nbal_drawdown -> stres orani (desiller):")
print(y.groupby(pd.qcut(train["bal_drawdown"],10,duplicates="drop")).agg(["mean","count"]).round(3).to_string())
print(f"\n2D: cokus orani (satir) x gelir soku ceyrekleri (sutun) — hucre=stres orani")
r_b=pd.cut(r,[0,0.6,0.8,1.0,10]); i_b=pd.qcut(train["income_shock"],4,duplicates="drop")
print(pd.crosstab(r_b,i_b,values=y,aggfunc="mean").round(3).to_string())
print(f"\n2D: cokus orani x aktivite ceyrekleri")
a_b=pd.qcut(train["x_90_d_activity_rate"],4,duplicates="drop")
print(pd.crosstab(r_b,a_b,values=y,aggfunc="mean").round(3).to_string())

print("\n"+"="*70)
print("E0) BASELINE (mevcut ozellik seti, 5-fold, seed 42)")
print("="*70)
oof0=run_cv(X)
lb0=lb_score(y,oof0)
print(f"E0 | LogLoss {log_loss(y,oof0):.5f} | AUC {roc_auc_score(y,oof0):.5f} | LB {lb0:.5f}")

print("\n"+"="*70)
print("E2) ESIK/KINK OZELLIKLERI")
print("="*70)
def add_kinks(d):
    d=d.copy(); r=d["bal_recent_over_old"]
    kf={}
    for t in [0.9,0.8,0.7,0.6,0.5]:
        kf[f"drop_gt_{int((1-t)*100)}"]=(r<t).astype(int)     # %10..%50 dusus bayraklari
    kf["drop_depth"]=(1-r).clip(lower=0)                      # dusus derinligi (0'da kirilir)
    kf["drop_depth_sq"]=kf["drop_depth"]**2
    kf["drop_x_shock"]=kf["drop_depth"]*(1/(d["income_shock"]+0.1))
    kf["drop_x_lowact"]=kf["drop_depth"]*(1-d["x_90_d_activity_rate"])
    kf["min3_over_old"]=d["bal_min_recent_over_old"].clip(upper=1.5)
    return pd.concat([d,pd.DataFrame(kf,index=d.index)],axis=1)
X2=add_kinks(X); X2t=add_kinks(X_test)
oof2=run_cv(X2)
lb2=lb_score(y,oof2)
print(f"E2 | LogLoss {log_loss(y,oof2):.5f} | AUC {roc_auc_score(y,oof2):.5f} | LB {lb2:.5f} | delta {lb2-lb0:+.5f}")

print("\n"+"="*70)
print("E3) PSEUDO-LABELING (test'in emin tahminleri, fold-ici)")
print("="*70)
oof3=run_cv(X, Xtd=X_test, pseudo=True)
lb3=lb_score(y,oof3)
print(f"E3 | LogLoss {log_loss(y,oof3):.5f} | AUC {roc_auc_score(y,oof3):.5f} | LB {lb3:.5f} | delta {lb3-lb0:+.5f}")

print("\n"+"="*70)
print("E4) AUC-DURDURAN IKIZ + KARISIM")
print("="*70)
oof4=run_cv(X, auc_stop=True)
lb4a=lb_score(y,oof4)
mix=0.5*oof0+0.5*oof4
lb4=lb_score(y,mix)
print(f"E4 tek   | LB {lb4a:.5f} | delta {lb4a-lb0:+.5f}")
print(f"E4 karisim(E0+E4) | LogLoss {log_loss(y,mix):.5f} | AUC {roc_auc_score(y,mix):.5f} | LB {lb4:.5f} | delta {lb4-lb0:+.5f}")

print("\n"+"="*70)
print("OZET (delta = E0 baseline'a gore)")
print("="*70)
for name,lb_ in [("E0 baseline",lb0),("E2 kink",lb2),("E3 pseudo",lb3),("E4 karisim",lb4)]:
    print(f"  {name:12s}: LB {lb_:.5f}  ({lb_-lb0:+.5f})")
print("\nBu ozet + E1 tablolarini oldugu gibi yapistir -> kazananlari v15'e koyacagiz.")
