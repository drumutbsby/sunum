...
# ===================== LAB-2: PURUZSUZ MODEL AVI ====================
# E1 bulgusu: uretec PURUZSUZ bir fonksiyon (kirilma yok, 2 ana boyut).
# Agaclar puruzsuz egriyi merdivenle taklit eder -> ~0.01 AUC kaybi.
# Adaylar: L1 linear-tree LGB, L2 spline-lojistik (GAM), L4 log1p-MLP.
# Hepsi ayni 5-fold seed42; sonda hepsinin STACK kombinasyonlari.
# =====================================================================
...
def report(tag, oof, ref=None):
    lb = lb_score(y, oof)
    d = f" | delta {lb-ref:+.5f}" if ref is not None else ""
    print(f"{tag} | LogLoss {log_loss(y,oof):.5f} | AUC {roc_auc_score(y,oof):.5f} | LB {lb:.5f}{d}")
    return lb

# ---- E0: baseline GBM (referans) ------------------------------------
print("\nE0 baseline kosuyor...")
oof0 = np.zeros(len(X))
for tr, va in FOLDS:
    m = lgb.train(dict(BEST_LGB, metric=["binary_logloss","auc"], seed=42),
        lgb.Dataset(X.iloc[tr], y.iloc[tr]), num_boost_round=6000,
        valid_sets=[lgb.Dataset(X.iloc[va], y.iloc[va])],
        callbacks=[lgb.early_stopping(200, first_metric_only=True), lgb.log_evaluation(0)])
    oof0[va] = m.predict(X.iloc[va])
lb0 = report("E0 GBM      ", oof0)

# ---- L1: LINEAR-TREE LGB (yapraklarda dogrusal model) ----------------
print("\nL1 linear-tree kosuyor...")
p1 = dict(BEST_LGB, metric=["binary_logloss","auc"], seed=42,
          linear_tree=True, lambda_l2=5.0, num_leaves=16)
oof1 = np.zeros(len(X))
for tr, va in FOLDS:
    m = lgb.train(p1, lgb.Dataset(X.iloc[tr], y.iloc[tr]), num_boost_round=6000,
        valid_sets=[lgb.Dataset(X.iloc[va], y.iloc[va])],
        callbacks=[lgb.early_stopping(200, first_metric_only=True), lgb.log_evaluation(0)])
    oof1[va] = m.predict(X.iloc[va])
lb1 = report("L1 lin-tree ", oof1, lb0)

# ---- L2: SPLINE-LOJISTIK (GAM tarzi — puruzsuz fonksiyon geri kazanimi)
SMOOTH = ["bal_recent_over_old","bal_min_recent_over_old","bal_slope6_norm","bal_drawdown",
          "bal_last_vs_mean","bal_cv","bal_min_to_mean","bal_last_vs_max",
          "income_shock","income_cv","x_90_d_activity_rate","outflow_ratio",
          "m1_outflow_ratio","spend_to_inflow","withdraw_to_deposit","send_to_received",
          "bal_decline_streak","bal_argmin_month","total_active_months","grp_size",
          "income_mean","bal_mean","bal_last","arpu","net_flow","m1_net_flow"]
SMOOTH = [c for c in SMOOTH if c in X.columns]
LOG1P  = {"income_mean","bal_mean","bal_last","arpu"}

def smooth_matrix(df):
    M = df[SMOOTH].copy()
    for c in LOG1P & set(M.columns):
        M[c] = np.log1p(M[c].clip(lower=0))
    for c in ["net_flow","m1_net_flow"]:
        if c in M: M[c] = np.sign(M[c])*np.log1p(M[c].abs())
    return M

# etkilesim carpimlari (E1 2D tablolarinin gosterdigi ciftler)
def add_inter(M):
    M = M.copy()
    M["ix_drop_shock"] = M["bal_recent_over_old"]*M["income_shock"].clip(upper=5)
    M["ix_drop_act"]   = M["bal_recent_over_old"]*M["x_90_d_activity_rate"]
    M["ix_slope_shock"]= M["bal_slope6_norm"]*M["income_shock"].clip(upper=5)
    M["ix_dd_shock"]   = M["bal_drawdown"]*M["income_shock"].clip(upper=5)
    return M

print("\nL2 spline-lojistik kosuyor...")
Msm = add_inter(smooth_matrix(X))
oof2 = np.zeros(len(X))
for tr, va in FOLDS:
    lo, hi = Msm.iloc[tr].quantile(0.005), Msm.iloc[tr].quantile(0.995)
    Ztr = Msm.iloc[tr].clip(lo, hi, axis=1); Zva = Msm.iloc[va].clip(lo, hi, axis=1)
    sp = SplineTransformer(n_knots=12, degree=3, include_bias=False).fit(Ztr)
    sc = StandardScaler().fit(sp.transform(Ztr))
    lr = LogisticRegression(C=0.5, max_iter=3000)
    lr.fit(sc.transform(sp.transform(Ztr)), y.iloc[tr])
    oof2[va] = lr.predict_proba(sc.transform(sp.transform(Zva)))[:,1]
oof2 = np.clip(oof2, 1e-6, 1-1e-6)
lb2 = report("L2 spline-LR", oof2, lb0)

# ---- L4: log1p-MLP (ilk MLP'nin agir-kuyruk sorunu giderilmis hali) --
print("\nL4 log1p-MLP kosuyor...")
Xf = X.copy()
...
for tr, va in FOLDS:
    ...
    m = MLPClassifier(hidden_layer_sizes=(512,256,128), alpha=3e-4,
        learning_rate_init=1e-3, batch_size=1024, max_iter=300,
        early_stopping=True, n_iter_no_change=20, validation_fraction=0.1,
        random_state=42)
    m.fit(sc.transform(Xf[tr]), y.iloc[tr])
    oof4[va] = m.predict_proba(sc.transform(Xf[va]))[:,1]
oof4 = np.clip(oof4, 1e-6, 1-1e-6)
lb4 = report("L4 log1p-MLP", oof4, lb0)
...
print("\nSTACK kombinasyonlari:")
combos = {
    "GBM+linTree           ": [oof0, oof1],
    "GBM+splineLR          ": [oof0, oof2],
    "GBM+MLP               ": [oof0, oof4],
    "GBM+linTree+splineLR  ": [oof0, oof1, oof2],
    "GBM+linTree+splineLR+MLP": [oof0, oof1, oof2, oof4],
}
results = {}
for name, oo in combos.items():
    results[name] = report("  "+name, stack(oo), lb0)

print("\n" + "="*68)
print(f"OZET | E0 referans LB: {lb0:.5f}")
for name, lb in sorted(results.items(), key=lambda kv: -kv[1]):
    print(f"  {name}: {lb:.5f} ({lb-lb0:+.5f})")
print("En iyi kombo v15'in iskeleti olacak — ciktiyi oldugu gibi yapistir.")
