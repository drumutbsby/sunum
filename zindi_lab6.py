...
def add_fp_balsigma(d):
    d=d.copy(); ff={}
    bl=d["bal_last"]; drift=d["net_recent3"]
    sigb=d["bal_std"].clip(lower=1.0)
    sigc=np.sqrt(d["net_std6"].clip(lower=1.0)**2 + sigb**2)
    for tag,sig in [("bsig",sigb),("csig",sigc)]:
        z=((0-(bl+drift))/sig).clip(-8,8)
        ff[f"fp_z_{tag}"]=z; ff[f"fp_p_{tag}"]=norm.cdf(z)
    return pd.concat([d,pd.DataFrame(ff,index=d.index)],axis=1)

print("\nE0 (v17 seti) kosuyor...")
lb0,_ = run_cv(Xb17, "E0 v17-set  ")

print("\nQ1 (+runway) kosuyor...")
X1 = add_runway(Xb17)
lb1,_ = run_cv(X1, "Q1 +runway  ")
print(f"   delta: {lb1-lb0:+.5f}")

print("\nQ1+Q2 (+sok) kosuyor...")
X2 = add_shock(X1)
lb2,_ = run_cv(X2, "Q2 +sok     ")
print(f"   delta: {lb2-lb0:+.5f}")

print("\nQ1+Q2+Q3 (+alt-sigma fp) kosuyor...")
X3 = add_fp_balsigma(X2)
lb3,_ = run_cv(X3, "Q3 +altsigma")
print(f"   delta: {lb3-lb0:+.5f}")

print("\n"+"="*68)
print(f"OZET | E0: {lb0:.5f} | Q1: {lb1-lb0:+.5f} | Q2(kum): {lb2-lb0:+.5f} | Q3(kum): {lb3-lb0:+.5f}")
print("Pozitif katmanlar v18'e girer. Ciktiyi oldugu gibi yapistir.")
