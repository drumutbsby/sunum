...
try:
    import dataiku
    train = dataiku.Dataset("Train_1").get_dataframe()
...
# ===================== LAB-4: NET AKIS SERISI + FIRST-PASSAGE ========
# Hipotez: uretec "bakiye + gelecek net akis -> batma olasiligi" simule
# ediyor. Bugune kadar her AILEYI ayri isledik ama AYLIK NET AKIS
# SERISINI (net_m = o ayin gelen-gideni) hic kurmadik; m1 haric.
#   N1: net_m1..m6 serisi + egim/oran/negatif-ay/kumulatif ozellikleri
#   N2: first-passage olasiliklari: P(bakiye esigi asagi keser) =
#       Phi((esik - bal_son - drift*h) / (sigma*sqrt(h)))
# Referans E0 ayni fold'larla yeniden olculur. Sure: ~15-20 dk (40 vCPU)
# =====================================================================
...
def run_cv(Xd, tag):
    oof=np.zeros(len(Xd))
    for tr,va in FOLDS:
        m=lgb.train(dict(BEST_LGB, metric=["binary_logloss","auc"], seed=42),
            lgb.Dataset(Xd.iloc[tr],y.iloc[tr]),num_boost_round=6000,
            valid_sets=[lgb.Dataset(Xd.iloc[va],y.iloc[va])],
            callbacks=[lgb.early_stopping(200,first_metric_only=True),lgb.log_evaluation(0)])
        oof[va]=m.predict(Xd.iloc[va])
    lb=lb_score(y,oof)
    print(f"{tag} | LogLoss {log_loss(y,oof):.5f} | AUC {roc_auc_score(y,oof):.5f} | LB {lb:.5f}")
    return lb, oof

# ---- N1: aylik NET AKIS serisi ---------------------------------------
IN_F  = ["received","deposit","transfer_from_bank"]
OUT_F = ["mm_send","withdraw","paybill","merchantpay"]
...
# ===================== LAB-5: NET-AKIS DAMARINI DERINLESTIR =========
# v16 kazanci dogrulandi (+0.0027 tam pipeline). Simdi ayni damardan
# uc katman daha:
#   P1: AYLIK OUT/IN ORAN SERISI (out_m/in_m, m1..m6) + egim + >1 sayaclari
#       + net ivme (net_m1-net_m2 vb.)
#   P2: COK-UFUKLU first-passage (h=2,3 ay) + ek esikler (0.10, 0.75)
#       + PROJEKSIYON: onumuzdeki 3 ayin tahmini minimum bakiyesi
#   P3: fp x gelir/aktivite/oynaklik ETKILESIMLERI
# Taban (E0) = v16 ozellik seti (net+fp DAHIL). Kumulatif delta olculur.
# Sure: ~25 dk (40 vCPU)
# =====================================================================

# E0 tabani: v16 seti = X + net-seri + first-passage
Xb = add_first_passage(add_net_series(X))
...
def add_fp_inter(d):
    d=d.copy(); it={}
    it["fpz_x_lowshock"]=d["fp_z_zero"]*(1/(d["income_shock"].clip(upper=5)+0.1))
    it["fpz_x_cv"]      =d["fp_z_zero"]*d["income_cv"].clip(upper=5)
    it["fpz_x_lowact"]  =d["fp_z_zero"]*(1-d["x_90_d_activity_rate"])
    it["fpp_x_balcv"]   =d["fp_p_zero"]*d["bal_cv"].clip(upper=5)
    return pd.concat([d,pd.DataFrame(it,index=d.index)],axis=1)

# ===================== LAB-6: RUNWAY + SOK + ALT-SIGMA ===============
# Taban (E0) = v17 seti (net+fp+oran+cokufuk; P3 haric).
#   Q1: RUNWAY — bakiye / aylik gider (kac aylik nefes?) + cikis serisi
#   Q2: TEK-ISLEM SOKU — en buyuk tek cikis / bakiye + konsantrasyon
#   Q3: fp'yi BAKIYE oynakligiyla (bal_std) ve birlesik sigma ile kur
# Sure: ~20-25 dk (40 vCPU)
# =====================================================================

# E0 tabani: v17 seti
Xb17 = add_fp_multi(add_ratio_series(Xb))

# ===================== LAB-7: BOOSTING MODU CESITLILIGI ==============
# Son denenmemis model ailesi: ayni ozellik setinde FARKLI boosting
# rejimleri. Amac tek basina kazanmak degil -> STACK'e yeni ses katmak.
#   D1: LGB DART  (agac birakmali — asiri uyumu kirar)
#   D2: LGB GOSS  (gradyan-tabanli ornekleme)
#   D3: LGB extra_trees (asiri rastgele bolmeler)
# Her biri E0 ile stack'lenir; en iyi kombo raporlanir.
# Sure: ~30-35 dk (40 vCPU)
# =====================================================================
from sklearn.linear_model import LogisticRegression

def stack_oofs(oofs):
    Z=np.column_stack([np.log(np.clip(o,1e-6,1-1e-6)/(1-np.clip(o,1e-6,1-1e-6))) for o in oofs])
    out=np.zeros(len(y))
    for tr,va in FOLDS:
...
print("\nE0 (v17 seti, gbdt) kosuyor...")
lb0, oof0 = run_cv(Xb17, "E0 gbdt      ")

# D1: DART — erken durdurma guvenilmez, sabit tur kullan
print("\nD1 DART kosuyor...")
p1=dict(BEST_LGB, boosting="dart", drop_rate=0.1, skip_drop=0.5,
        learning_rate=0.03, metric=["binary_logloss","auc"], seed=42)
oof1=np.zeros(len(y))
for tr,va in FOLDS:
    m=lgb.train(p1,lgb.Dataset(Xb17.iloc[tr],y.iloc[tr]),num_boost_round=1200,
        valid_sets=[lgb.Dataset(Xb17.iloc[va],y.iloc[va])],
        callbacks=[lgb.log_evaluation(0)])
    oof1[va]=m.predict(Xb17.iloc[va])
lb1=lb_score(y,oof1)
print(f"D1 dart      | LogLoss {log_loss(y,oof1):.5f} | AUC {roc_auc_score(y,oof1):.5f} | LB {lb1:.5f}")

# D2: GOSS
print("\nD2 GOSS kosuyor...")
p2=dict(BEST_LGB, boosting="goss", metric=["binary_logloss","auc"], seed=42)
p2.pop("bagging_fraction",None); p2.pop("bagging_freq",None)   # goss bagging kullanmaz
oof2=np.zeros(len(y))
for tr,va in FOLDS:
    m=lgb.train(p2,lgb.Dataset(Xb17.iloc[tr],y.iloc[tr]),num_boost_round=6000,
        valid_sets=[lgb.Dataset(Xb17.iloc[va],y.iloc[va])],
        callbacks=[lgb.early_stopping(200,first_metric_only=True),lgb.log_evaluation(0)])
    oof2[va]=m.predict(Xb17.iloc[va])
lb2=lb_score(y,oof2)
print(f"D2 goss      | LogLoss {log_loss(y,oof2):.5f} | AUC {roc_auc_score(y,oof2):.5f} | LB {lb2:.5f}")

# D3: extra_trees
print("\nD3 extra_trees kosuyor...")
p3=dict(BEST_LGB, extra_trees=True, metric=["binary_logloss","auc"], seed=42)
oof3=np.zeros(len(y))
for tr,va in FOLDS:
    m=lgb.train(p3,lgb.Dataset(Xb17.iloc[tr],y.iloc[tr]),num_boost_round=6000,
        valid_sets=[lgb.Dataset(Xb17.iloc[va],y.iloc[va])],
        callbacks=[lgb.early_stopping(200,first_metric_only=True),lgb.log_evaluation(0)])
    oof3[va]=m.predict(Xb17.iloc[va])
lb3=lb_score(y,oof3)
print(f"D3 xtrees    | LogLoss {log_loss(y,oof3):.5f} | AUC {roc_auc_score(y,oof3):.5f} | LB {lb3:.5f}")

print("\nSTACK kombinasyonlari:")
res={}
for name,oo in [("E0+dart",[oof0,oof1]),("E0+goss",[oof0,oof2]),("E0+xtrees",[oof0,oof3]),
                ("E0+dart+goss",[oof0,oof1,oof2]),("hepsi",[oof0,oof1,oof2,oof3])]:
    s_=stack_oofs(oo); lbx=lb_score(y,s_)
    res[name]=lbx
    print(f"  {name:14s} | LB {lbx:.5f} | delta {lbx-lb0:+.5f}")

print("\n"+"="*68)
print(f"OZET | E0: {lb0:.5f}")
for k,v in sorted(res.items(),key=lambda kv:-kv[1]):
    print(f"  {k:14s}: {v:.5f} ({v-lb0:+.5f})")
print("Delta >= +0.001 olan varsa v19'a girer; yoksa SAVUNMA moduna geceriz.")
