...
ALL_F = IN_F + OUT_F
CNT = {"paybill":"companies","merchantpay":"merchants","transfer_from_bank":"banks",
       "mm_send":"recipients","received":"senders","deposit":"agents","withdraw":"agents"}

# ---- R1: bakiye-akis mutabakati -------------------------------------
def add_reconcile(d):
    d = d.copy(); f = {}
    net = np.column_stack([d[f"net_m{m}"].values for m in range(1,7)])   # kolon0 = m1 (yeni)
    bal = np.column_stack([d[c].values for c in BAL])
    # residual_m = (bal_m - bal_{m+1}) - net_m   , m = 1..5
    R = np.column_stack([(bal[:,i] - bal[:,i+1]) - net[:,i] for i in range(5)])
    f["recon_mean"]   = R.mean(axis=1)
    f["recon_std"]    = R.std(axis=1)
    f["recon_last"]   = R[:,0]
    f["recon_min"]    = R.min(axis=1)
    f["recon_max"]    = R.max(axis=1)
    f["recon_neg_cnt"]= (R < 0).sum(axis=1)
    f["recon_slope"]  = R @ (np.array([2.0,1.0,0.0,-1.0,-2.0])/10.0)
    scale = np.abs(net).mean(axis=1) + 1.0
    f["recon_mean_norm"] = R.mean(axis=1)/scale        # akis olcegine gore
    f["recon_last_norm"] = R[:,0]/scale
    bmean = bal.mean(axis=1) + 1.0
    f["recon_mean_vs_bal"] = R.mean(axis=1)/bmean      # bakiye olcegine gore
    # aciklanma orani: bakiye degisiminin ne kadari akisla aciklaniyor?
    dbal = bal[:,:5] - bal[:,1:]
    f["recon_expl_ratio"] = (np.abs(net[:,:5]).sum(axis=1))/(np.abs(dbal).sum(axis=1)+1.0)
    f["recon_corr"] = np.array([                       # net ile dbal korelasyonu
        np.corrcoef(net[i,:5], dbal[i])[0,1] if np.std(net[i,:5])>0 and np.std(dbal[i])>0 else 0.0
        for i in range(len(d))])
    return pd.concat([d, pd.DataFrame(f, index=d.index)], axis=1)

# ---- R2: aileler-arasi aylik toplam serileri ------------------------
def add_cross_family(d):
    d = d.copy(); f = {}
    W = np.array([2.5,1.5,0.5,-0.5,-1.5,-2.5])/17.5
    vol  = np.column_stack([sum(d[f"m{m}_{tx}_volume"] for tx in ALL_F).values for m in range(1,7)])
    cpty = np.column_stack([sum(d[f"m{m}_{tx}_{CNT[tx]}"] for tx in ALL_F).values for m in range(1,7)])
    dvst = np.column_stack([sum((d[f"m{m}_{tx}_volume"]>0).astype(int) for tx in ALL_F).values for m in range(1,7)])
    for tag, M in [("txn", vol), ("cpty", cpty), ("divers", dvst)]:
        f[f"tot_{tag}_mean"]   = M.mean(axis=1)
        f[f"tot_{tag}_std"]    = M.std(axis=1)
        f[f"tot_{tag}_last"]   = M[:,0]
        f[f"tot_{tag}_slope"]  = M @ W
        f[f"tot_{tag}_rec_over_old"] = (M[:,:3].mean(axis=1)+1)/(M[:,3:].mean(axis=1)+1)
        f[f"tot_{tag}_min"]    = M.min(axis=1)
    f["txn_per_cpty"]   = vol.sum(axis=1)/(cpty.sum(axis=1)+1)     # muhatap basina islem
    f["value_per_txn"]  = (d["total_inflow"]+d["total_outflow"])/(vol.sum(axis=1)+1)
    return pd.concat([d, pd.DataFrame(f, index=d.index)], axis=1)
...
print(f"\nEleme kazanani: {best_tag} (LB {cands[best_tag][0]:.5f}, delta {cands[best_tag][0]-lb0:+.5f})")

print("\n" + "="*70)
print("TANI: GroupKFold(musteri) vs rastgele KFold — CV'miz iyimser mi?")
print("="*70)
g = train["cust_id"].values
lb_g, _ = eval_cv(X, "E0 GroupKFold(musteri)", groups=g)
print(f"   rastgele KFold: {lb0:.5f} | GroupKFold: {lb_g:.5f} | fark: {lb_g-lb0:+.5f}")
print("   (fark cok negatifse CV'miz iyimser, model secimimiz yanli olabilir)")

print("\n" + "="*70)
print("ASAMA 2: DOGRULAMA (10-fold x 3 seed) — URETIM kosullari")
print("="*70)
SEEDS3 = (42, 2024, 777)
lb0v, _ = eval_cv(X, "E0 dogrulama", n_folds=10, seeds=SEEDS3)
lbbv, _ = eval_cv(cands[best_tag][1], f"{best_tag} dogrulama", n_folds=10, seeds=SEEDS3)
print(f"\n>>> URETIM KOSULLARINDA DELTA: {lbbv-lb0v:+.5f}")
print("    (>= +0.0015 ise v20'ye tasinir; degilse ELENIR — dorduncu bos tura girmeyiz)")
print("="*70)
