# =====================================================================
# Zindi — Financial Stress Prediction  |  v10 (EDA-YONLENDIRMELI TUR)
#
# EDA bulgulari ve karsiliklari:
#   B1) Stresin ana sinyali SON 3 AYDA BAKIYE COKUSU (stresli medyan:
#       m4-m6 ~4500 -> m1-m3 ~3400; stressiz duz ~5100-5400)
#       -> bakiye EGIMI (slope), oran-bazli dusus, drawdown ozellikleri
#   B2) En korele ham kolonlar deposit_agents / received_senders /
#       withdraw_agents -> bu ailelere ozel recent/old ORANLARI
#   B3) Stresliler: dusuk arpu + dusuk aktivite -> arpu-normalize bakiye
#   B4) Musteri ici etiketler Binom(4,0.15) ile birebir = bagimsiz ->
#       cust_te'ler cop; ama ayni musterinin DAVRANIS kiyasi hala gecerli
#       -> musteri-ici yuzdelik SIRA (rank) ozellikleri
#   B5) Train/test kaymasi SIFIR -> CV'ye guven tam
#
# Ayrica (ucretli Colab gucuyle):
#   - LGB Optuna YENIDEN ACIK (eski parametreler ESKI veriye gore idi):
#     LGB_TRIALS=40, CAT_TRIALS=25
#   - SEEDS 5'e cikti
#   - 4. aday: STACK+KALIBRE (stack ciktisi ayrica kalibre edilir)
#
# Gecilecek hedef (v9): LogLoss 0.25243 | AUC 0.90397 | LB 0.10916
#
# Colab: !pip install catboost lightgbm optuna -q
# Sure (8 vCPU): aramalar ~35-45 dk + final ~30 dk. Arka planda
#   yurutmeyi ac. Zaman darsa LGB_TRIALS=15, CAT_TRIALS=10, SEEDS ilk 3.
# =====================================================================
...
# ---------------- 6a) OPTUNA: LGB aramasi (YENI veri + YENI ozellikler
# icin yeniden aciliyor — eski gomulu parametreler eski veriye aitti) --
import optuna
optuna.logging.set_verbosity(optuna.logging.WARNING)
LGB_TRIALS = 40   # zaman darsa 15
CAT_TRIALS = 25   # zaman darsa 10
...
def lgb_objective(trial):
    p = {"objective":"binary","metric":"binary_logloss","verbose":-1,"seed":42,
         ...
         "feature_fraction":  trial.suggest_float("feature_fraction",0.45,0.95),
    ...
    skf_o = StratifiedKFold(3, shuffle=True, random_state=42)
    lls = []
    for i,(tr,va) in enumerate(skf_o.split(X,y)):
...
print(f"\nLGB icin Optuna araniyor ({LGB_TRIALS} deneme)...")
lgb_study = optuna.create_study(direction="minimize",
        sampler=optuna.samplers.TPESampler(seed=42),
        pruner=optuna.pruners.MedianPruner(n_warmup_steps=1))
lgb_study.optimize(lgb_objective, n_trials=LGB_TRIALS)
print("LGB en iyi 3-fold LogLoss:", round(lgb_study.best_value,5))
print("LGB en iyi parametreler:", lgb_study.best_params)
BEST_LGB = dict({"objective":"binary","verbose":-1,"bagging_freq":1}, **lgb_study.best_params)

# ---------------- 6a2) OPTUNA: CatBoost aramasi ----------------------
...
print(f"CatBoost icin Optuna araniyor ({CAT_TRIALS} deneme)...")
cat_study = optuna.create_study(direction="minimize",
        sampler=optuna.samplers.TPESampler(seed=42),
        pruner=optuna.pruners.MedianPruner(n_warmup_steps=1))
cat_study.optimize(cat_objective, n_trials=CAT_TRIALS)
print("CAT en iyi 3-fold LogLoss:", round(cat_study.best_value,5))
print("CAT en iyi parametreler:", cat_study.best_params)
...
def cv_cat_seeds():
    ...
    for seed in SEEDS:
        skf=StratifiedKFold(5,shuffle=True,random_state=seed)
        for tr,va in skf.split(X,y):
            m=CatBoostClassifier(iterations=6000, loss_function="Logloss",
                eval_metric="Logloss", random_seed=seed,
                early_stopping_rounds=200, verbose=False, **BEST_CAT)
            m.fit(X.iloc[tr],y.iloc[tr],eval_set=(X.iloc[va],y.iloc[va]),use_best_model=True)
...
for wl in np.arange(0,1.0001,0.05):
    s=lb_score(y, wl*oof_l+(1-wl)*oof_c)
    if s>best[0]: best=(s,round(wl,2))
...
print(f"\nSecilen: {chosen}")
print(f">>> v10 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
print("    (v9 idi: LogLoss 0.25243 | AUC 0.90397 | LB 0.10916  <- gecilecek hedef)")
