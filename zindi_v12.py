# =====================================================================
# Zindi — Financial Stress Prediction  |  v12 (MLP UCUNCU SES)
#
# v11 uzerine: agac-olmayan bir model (MLP sinir agi) stack'e ucuncu
# ses olarak eklenir. Agaclar ve ag farkli hata desenleri uretir;
# lojistik stacker uc sesi optimal agirliklarla birlestirir.
#   - MLP: StandardScaler + (256,128) gizli katman, early stopping,
#     fold-ici olceklendirme (sizinti yok), 2 seed x 5 fold
#   - Blend artik 3 modelli grid; stack 3 girisli
#   - v11 etiket hatasi duzeltildi (v10 yaziyordu)
#
# Gecilecek hedef (v11): LogLoss 0.24347 | AUC 0.91095 | LB 0.12091
#
# Colab: !pip install catboost lightgbm -q
# Sure (8 vCPU): GBM'ler ~30 dk + MLP ~15-25 dk. Arka planda yurut.
#   Zaman darsa MLP_SEEDS=[42] yap.
# =====================================================================
...
BEST_CAT = {
    "learning_rate":       0.030215521482191397,
    "depth":               6,
...
MLP_SEEDS = [42, 2024]   # zaman darsa [42]

def cv_mlp_seeds():
    oof=np.zeros(len(X)); tp=np.zeros(len(X_test))
    Xv = X.values.astype(np.float64); Xt = X_test.values.astype(np.float64)
    for seed in MLP_SEEDS:
        skf=StratifiedKFold(5,shuffle=True,random_state=seed)
        for tr,va in skf.split(Xv,y):
            sc=StandardScaler().fit(Xv[tr])          # olcek fold-icinden (sizinti yok)
            m=MLPClassifier(hidden_layer_sizes=(256,128), alpha=1e-3,
                learning_rate_init=1e-3, batch_size=512, max_iter=200,
                early_stopping=True, n_iter_no_change=15,
                validation_fraction=0.1, random_state=seed)
            m.fit(sc.transform(Xv[tr]), y.iloc[tr])
            oof[va]+=m.predict_proba(sc.transform(Xv[va]))[:,1]/len(MLP_SEEDS)
            tp    +=m.predict_proba(sc.transform(Xt))[:,1]/(len(MLP_SEEDS)*5)
        print(f"    MLP seed {seed} ok")
    return np.clip(oof,1e-6,1-1e-6), np.clip(tp,1e-6,1-1e-6)
...
print("MLP...");       oof_m,tp_m = cv_mlp_seeds()
for name,o in [("LGB",oof_l),("CAT",oof_c),("MLP",oof_m)]:
...
for wl in np.arange(0,1.0001,0.05):
    for wc in np.arange(0,1.0001-wl,0.05):
        wm=1-wl-wc
        sc_=lb_score(y, wl*oof_l+wc*oof_c+wm*oof_m)
        if sc_>best[0]: best=(sc_,(round(wl,2),round(wc,2),round(wm,2)))
wl,wc,wm = best[1]
oof_b = wl*oof_l+wc*oof_c+wm*oof_m
tp_b  = wl*tp_l +wc*tp_c +wm*tp_m
print(f"\nEn iyi blend -> LGB:{wl} CAT:{wc} MLP:{wm}")
...
oof_s, tp_s = cross_fit_stack([oof_l, oof_c, oof_m], y, [tp_l, tp_c, tp_m])
...
print(f"\nSecilen: {chosen}")
print(f">>> v12 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
print("    (v11 idi: LogLoss 0.24347 | AUC 0.91095 | LB 0.12091  <- gecilecek hedef)")
