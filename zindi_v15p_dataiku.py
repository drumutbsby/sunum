# =====================================================================
# Zindi — v15p DATAIKU SURUMU (paralel yavas-uzun cekirdek)
#
# Veri kaynagi: DSS dataset'leri ("Train_1" ve "Test" — adin farkliysa
# asagidaki DS_TRAIN / DS_TEST sabitlerini degistir).
# SampleSubmission dataset'i varsa kullanilir; yoksa test ID'lerinden
# guvenli sekilde olusturulur (format: ID,Target).
#
# CIKTI: submission
#   1) once "submissions" adli Managed Folder denenir (Flow'dan indir)
#   2) yoksa notebook'un calistigi diske submission.csv yazilir
#
# 40 vCPU beklenen sure: ~45-75 dk (10 isci x 4 thread otomatik)
...
print(f"\nSecilen: {chosen}")
print(f">>> v15p PARALEL CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
...
try:
    ...
    print(f">> Managed Folder bulunamadi ({type(e).__name__}) — yerel diske yazildi: submission.csv")
    print("   Notebook'tan indirmek icin yeni hucrede:")
    print("   from IPython.display import FileLink; FileLink('submission.csv')")
