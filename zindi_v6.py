# =====================================================================
# Zindi — Financial Stress Prediction  |  v6  (MUSTERI KIMLIGI TURU)
#
# KESIF: Ayni musteri (ayni arpu/age/gender/region/... profili) hem
# train'de hem test'te birden fazla snapshot ile geciyor. Musterinin
# train'deki ETIKETLI snapshot'lari, test'teki satirlarini tahmin etmede
# cok guclu sinyal. Tamamen yarisma verisi ici -> kurallara uygun.
#
# v5'in ustune eklenenler:
#   1) cust_id: profil parmak izi (8 profil kolonu birlestirilir)
#   2) OOF musteri target-encoding (kendi etiketi ASLA kendi te'sine
#      girmez -> sizinti yok; smoothing prior=20)
#   3) Musteri-grubu davranis ozellikleri (train+test, etiketsiz):
#      grup ortalama bakiye/akis + satirin gruptan sapmasi
#   4) Ayni ensemble (seed-avg LGB + CAT + XGB) + isotonic kalibrasyon
#
# Colab: !pip install catboost xgboost lightgbm -q
# Sure: SEEDS=[42,2024] ile ~25 model (~8-12 dk). Final icin 777 ekle.
# =====================================================================
...
y_tmp = train[TARGET]
te_tr, te_te = oof_target_encode(train["cust_id"], y_tmp, test["cust_id"])
train["cust_te"] = te_tr
test["cust_te"]  = te_te
print(f"cust_te hazir | train aralik: [{te_tr.min():.3f},{te_tr.max():.3f}]")
...
# ---------------- 6) Ensemble (v5 ile ayni iskelet) ------------------
SEEDS = [42, 2024]          # final kosuda [42, 2024, 777] yap
...
for name,o in [("LGB",oof_l),("CAT",oof_c),("XGB",oof_x)]:
...
print(f"\nSecilen: {chosen}")
print(f">>> v6 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
