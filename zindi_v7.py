# =====================================================================
# Zindi — Financial Stress Prediction  |  v7  (v6 DUZELTMELERI)
#
# v6 teshisi: (1) prior=20 smoothing cust_te sinyalini ezdi (aralik
# 0.13-0.26'ya sikisti), (2) v6 2 seed kostu (v4 3 seed'di), (3) XGB
# blend agirligi iki kez 0.0 cikti -> tamamen atildi.
#
# v7 degisiklikleri:
#   1) cust_te iki priorla: prior=3 (keskin) + prior=20 (yumusak),
#      model hangisi faydaliysa onu kullanir
#   2) TANI CIKTISI: "te-alone OOF AUC" — musteri-etiket sinyalinin
#      gercek gucunu olcer (>0.60 ise sinyal var demektir)
#   3) SEEDS=[42,2024,777] (v4 ile esit kosullar)
#   4) XGB kaldirildi -> zaman 3. seed'e harcaniyor
#   5) Kalibrasyon kaldi ama v6'da HAM kazandigi icin muhtemelen yine HAM
#
# Colab: !pip install catboost lightgbm -q
# Sure: 3 seed x 5 fold x 2 model = 30 model (~10-14 dk)
# =====================================================================
...
# ---------------- 6) Ensemble: 3-seed LGB + 3-seed CAT ---------------
SEEDS = [42, 2024, 777]
...
print(f"\nSecilen: {chosen}")
print(f">>> v7 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
print("    (v4 idi: LogLoss 0.27838 | AUC 0.88368 | LB 0.07510)  <- gecilecek hedef")
print("    (v6 idi: LogLoss 0.27925 | AUC 0.88295 | LB 0.07394)")
