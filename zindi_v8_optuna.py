# =====================================================================
# Zindi — Financial Stress Prediction  |  v8  (OPTUNA TURU)
#
# v7 tanisi: te-alone OOF AUC = 0.5047 -> musteri-etiket sinyali YOK
# (ayni musterinin farkli snapshot'lari bagimsiz etiket aliyor).
# Musteri kimligi yolu kapandi -> kazanc kaynagi: hiperparametre.
#
# v8 degisiklikleri:
#   1) OPTUNA ile LGB hiperparametre aramasi (TPE + MedianPruner,
#      hizli olsun diye 3-fold; final egitim yine 5-fold x 3 seed)
#   2) Bulunan parametrelerle 3-seed LGB + 3-seed CAT blend + kalibrasyon
#   3) GUVENLIK: submission'da bos Target kalirsa kod DURUR
#      (veri guncellemesi/eski SampleSubmission uyusmazligina karsi)
#
# Colab: !pip install catboost lightgbm optuna -q
# Sure: Optuna ~20-30 dk (TRIALS=20) + final ~12 dk.
#       Zaman darsa TRIALS=10 yap.
# =====================================================================
...
# ---------------- 6a) OPTUNA: LGB hiperparametre aramasi -------------
import optuna
optuna.logging.set_verbosity(optuna.logging.WARNING)
TRIALS = 20   # zaman darsa 10'a dusur
...
def objective(trial):
    p = {"objective":"binary","metric":"binary_logloss","verbose":-1,"seed":42,
         ...
         "feature_fraction":  trial.suggest_float("feature_fraction",0.45,0.9),
    ...
    skf_o = StratifiedKFold(3, shuffle=True, random_state=42)   # hiz icin 3-fold
    lls = []
...
print(f"\nOptuna araniyor ({TRIALS} deneme)...")
study = optuna.create_study(direction="minimize",
        sampler=optuna.samplers.TPESampler(seed=42),
        pruner=optuna.pruners.MedianPruner(n_warmup_steps=1))
study.optimize(objective, n_trials=TRIALS)
print("Optuna en iyi 3-fold LogLoss:", round(study.best_value,5))
print("En iyi parametreler:", study.best_params)
BEST_LGB = dict({"objective":"binary","verbose":-1,"bagging_freq":1}, **study.best_params)
...
print(f"\nSecilen: {chosen}")
print(f">>> v8 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
print("    (v7 idi [YENI veri]: LogLoss 0.25814 | AUC 0.90024 | LB 0.10196)  <- gecilecek hedef")
