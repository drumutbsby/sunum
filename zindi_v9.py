# =====================================================================
# Zindi — Financial Stress Prediction  |  v9
#
# v8 uzerine iki iyilestirme:
#   1) CATBOOST'A OPTUNA: CAT hala varsayilan parametrelerdeydi ama
#      blend'de %30 agirligi var -> simdi o da aranarak ayarlaniyor
#      (LGB parametreleri v8'den gomulu, tekrar aranmiyor)
#   2) STACKING META-MODEL: lineer blend yerine OOF logitleri uzerinde
#      cross-fit lojistik regresyon. HAM blend / KALIBRE / STACK
#      adaylarindan LB metriginde en iyisi OTOMATIK secilir.
#
# Gecilecek hedef (v8): LogLoss 0.25723 | AUC 0.90271 | LB 0.10385
#
# ONEMLI: SampleSubmission.csv guncel (Test ile ayni satir sayisinda)
# olmali - degilse kod guvenlik kontrolunde durur.
#
# Colab: !pip install catboost lightgbm optuna -q
# Sure: CAT aramasi ~20-25 dk (CAT_TRIALS=12) + final ~15 dk.
#       Zaman darsa CAT_TRIALS=6 yap.
# =====================================================================
...
# ---------------- 6a) Optuna'nin buldugu parametreler (GOMULU) --------
# v8 aramasinin sonucu (20 deneme, 3-fold LogLoss 0.26351):
BEST_LGB = {
    "objective": "binary", "verbose": -1, "bagging_freq": 1,
    "learning_rate":     0.03964876064134411,
    "num_leaves":        24,
    "feature_fraction":  0.8862916998545396,
    "bagging_fraction":  0.9263016196691256,
    "min_child_samples": 23,
    "lambda_l1":         5.284384205738846,
    "lambda_l2":         0.05939923582689613,
}
print("\nLGB: Optuna parametreleri gomulu (arama atlandi).")

# ---------------- 6a2) OPTUNA: CatBoost aramasi ----------------------
import optuna
optuna.logging.set_verbosity(optuna.logging.WARNING)
CAT_TRIALS = 12   # zaman darsa 6'ya dusur
...
# ---------------- 6b) Ensemble: 3-seed LGB(tuned) + 3-seed CAT --------
SEEDS = [42, 2024, 777]
...
# ---- 3 aday arasindan LB metrigine gore otomatik secim ---------------
candidates = {
...
print(f"\nSecilen: {chosen}")
print(f">>> v9 CV | LogLoss: {log_loss(y,oof_f):.5f} | AUC: {roc_auc_score(y,oof_f):.5f} | LB: {lb_score(y,oof_f):.5f}")
print("    (v8 idi: LogLoss 0.25723 | AUC 0.90271 | LB 0.10385  <- gecilecek hedef)")
